#include "QuadMeshClass.h"



QuadMeshClass::QuadMeshClass(ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
	initBuffers(device);
}


QuadMeshClass::~QuadMeshClass()
{
	// Run parent deconstructor
	BaseMesh::~BaseMesh();
}

void QuadMeshClass::initBuffers(ID3D11Device* device)
{
	...
	vertices[0].position = XMFLOAT3(0.0f, 2.0f, 0.0f);  // Top left.
	...
	vertices[3].position = XMFLOAT3(4.0f, 0.0f, 0.0f);  // top right.
	vertices[3].texture = XMFLOAT2(1.0f, 0.0f);

	vertices[4].position = XMFLOAT3(4.0f, 2.0f, 0.0f);  // top right.
	vertices[4].texture = XMFLOAT2(1.0f, 0.0f);

	vertices[5].position = XMFLOAT3(2.0f, 2.0f, 0.0f);  // top right.
...
void QuadMeshClass::sendData(ID3D11DeviceContext* deviceContext, D3D_PRIMITIVE_TOPOLOGY top)
{
	unsigned int stride;
	unsigned int offset;
...
void QuadMeshClass::CreatePlane(VertexType_Colour* vertices, unsigned long* indices)
{
	//for (int x = 0; x < 2; x++)
	//{	
	//		// Load the vertex array with data.

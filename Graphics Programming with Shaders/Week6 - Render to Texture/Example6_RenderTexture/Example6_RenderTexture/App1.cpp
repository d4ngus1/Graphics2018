...
void App1::init(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight, Input *in, bool VSYNC, bool FULL_SCREEN)
{
	...
	orthoSphereMesh = new OrthoMesh(renderer->getDevice(), renderer->getDeviceContext(), screenWidth / 4, screenHeight / 4, 440, screenHeight / 2.7);
	...
	sphereTexture = new RenderTexture(renderer->getDevice(), screenWidth, screenHeight, SCREEN_NEAR, SCREEN_DEPTH);

	birdeyeCamera = new Camera;
	birdeyeCamera->setPosition(0.0f, 5.0f, -4.0f);
	birdeyeCamera->setRotation(0.0f, 0.0f, 50.0f);

	sphereCamera = new Camera;
	sphereCamera->setPosition(0.0f, 5.0f, -4.0f);
	sphereCamera->setRotation(0.0f, 0.0f, 50.0f);
...
{
	// Set the render target to be the render to texture and clear it
	sphereTexture->setRenderTarget(renderer->getDeviceContext());
	sphereTexture->clearRenderTarget(renderer->getDeviceContext(), 1.0f, 0.0f, 0.0f, 1.0f);
	
	// Get matrices
	sphereCamera->update();
	XMMATRIX worldMatrix = renderer->getWorldMatrix();
	XMMATRIX viewMatrix = sphereCamera->getViewMatrix();
	XMMATRIX projectionMatrix = renderer->getProjectionMatrix();
...
void App1::finalPass()
{
	...
	//new
	orthoSphereMesh->sendData(renderer->getDeviceContext());
	textureShader->setShaderParameters(renderer->getDeviceContext(), worldMatrix, orthoViewMatrix, orthoMatrix, sphereTexture->getShaderResourceView());
	textureShader->render(renderer->getDeviceContext(), orthoSphereMesh->getIndexCount());

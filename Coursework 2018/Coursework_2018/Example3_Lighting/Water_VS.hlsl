...
OutputType main(InputType input)
{
    ...
    input.position * 6;
    //created the waves in the sine graph
    input.position.y += height * (sin(frequency * (time + input.position.x)));

	//modify the normals to match the sine wave proportions 
    input.normal.x = 1 - cos(frequency * (input.position.x + time));
    input.normal.y = abs(cos((frequency / 2) * (input.position.x + time)));
